...
void CFoePlaneMiddle::InitFoe()
{
	...
	m_blood = FOE_MIDDLE_BLOOD(中等);
	m_showid = 3;
	m_points = FOE_MIDDLE_BLOOD(中等);
}
...
void CFoePlaneMiddle::ShowFoe()
{
	::putimage(m_x, m_y,
		...
		&m_foe,
		IMG_FOEMIDDLE_WIDTH, (3 - m_showid) * IMG_FOEMIDDLE_HEIGHT, //开始的位置，高度是根据情况改变的
		SRCPAINT);
	::putimage(m_x, m_y,
		IMG_FOEMIDDLE_WIDTH, IMG_FOEMIDDLE_HEIGHT,
		&m_foe,
		0, (3 - m_showid) * IMG_FOEMIDDLE_HEIGHT,//开始的位置，高度是根据情况改变的
		SRCAND);
}
